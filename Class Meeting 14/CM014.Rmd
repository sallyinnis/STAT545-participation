...
#this returns y = mx + b x is year, so return year is the "m" value and also the "b" value for the intercept. We need to shift the y axids to be able to start at year 1952 because of the negative intercept.
```
...
#This has corrected the intercept to start at 1952. 
```
...
```{r}
summary(my_lm) # use to get a summary about the output. you get st. err in this. 
```

```{r}
tidy(my_lm)
newmy_lm= augment(my_lm)
ggplot(newmy_lm, aes(.resid)) +
...
```{r}
#This is going to predict life expectanyc for years that don't have actual values. 
